...
export default function EditProductScreen({ navigation, route }: Props) {
  ...
  const [currentStock,  setCurrentStock]  = useState(String(product.currentStock));
  const [criticalLimit, setCriticalLimit] = useState(String(product.criticalLimit));
  ...
  const handleUpdate = async () => {
    ...
    try {
      ...
      const msg = err.response?.data?.message || 'Ürün güncellenemedi.';
          ...
          onPress: async () => {
            ...
            try {
              ...
              Alert.alert('Silindi', 'Ürün başarıyla silindi.', [
                { text: 'Tamam', onPress: () => navigation.navigate('UrunListesi') },
              ]);
            } catch {
  ...
  return (
    <KeyboardAvoidingView style={s.root} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
      ...
      <View style={s.header}>
        ...
        <Text style={s.headerTitle}>Ürünü Düzenle</Text>
      ...
      <ScrollView contentContainerStyle={s.content} keyboardShouldPersistTaps="handled" showsVerticalScrollIndicator={false}>
        <View style={s.card}>
          ...
          <TextInput style={s.input} value={barcode} onChangeText={setBarcode} placeholder="Barkod numarası" placeholderTextColor="#94A3B8" />
...
const s = StyleSheet.create({
  ...
  deleteBtn: { width: 38, height: 38, borderRadius: 10, backgroundColor: '#FEE2E2', alignItems: 'center', justifyContent: 'center' },
